...
Run-Mutation `
    -Name "M1: Remove SUPPLIER_READ on GET list" `
    -File "src\main\java\com\pos\suppliers\controller\SupplierController.java" `
    -Search "@PreAuthorize(`"hasAuthority('SUPPLIER_READ')`") // suppliers list" `
    -Replace "@PreAuthorize(`"permitAll()`") // suppliers list" `
    -Test "SupplierApiIntegrationTests#cashierWithoutSupplierPermissionIsForbidden"

Run-Mutation `
    -Name "M2: Skip duplicate supplier code check" `
    -File "src\main\java\com\pos\suppliers\service\SupplierService.java" `
    -Search "if (supplierRepository.existsBySupplierCode(request.supplierCode().trim())) {" `
    -Replace "if (false && supplierRepository.existsBySupplierCode(request.supplierCode().trim())) {" `
    -Test "SupplierApiIntegrationTests#duplicateSupplierCodeIsConflict"

Run-Mutation `
    -Name "M3: Skip supplier audit write" `
    -File "src\main\java\com\pos\suppliers\service\SupplierService.java" `
    -Search "auditRecorder.record(AuditEvent.of(" `
    -Replace "if (false) auditRecorder.record(AuditEvent.of(" `
    -Test "SupplierApiIntegrationTests#createWritesAudit"
